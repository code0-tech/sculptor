import {EditableJSONEntry} from "./DataTypeTypeInputComponent"
...
export const DataTypeTypeInputTreeComponent: React.FC<DataTypeJSONInputTreeComponentProps> = (props) => {
    const {
        object,
        parentKey,
        isRoot = !parentKey,
        ...
        return (
            ...
            >
                <Flex align="center" style={{gap: ".35rem", textWrap: "nowrap"}} aria-selected={isActive || undefined}
                    ...
                    {icon}
                    <Text
                        hierarchy="tertiary">{Array.isArray(value) ? "is a list of type" : "is a nested object"}</Text>
                </Flex>
    ...
    const renderNodes = Array.isArray(value) || (value && typeof value === 'object')
        ? Object.entries(value as Record<string, unknown>).map(([key, val]) => {
            ...
            const icon = isCollapsable ? (isCollapsed ? <IconChevronUp size={13}/> :
                <IconChevronDown size={13}/>) : null
            const label = isCollapsable ? (
                <Flex align="center" style={{gap: ".35rem", textWrap: "nowrap"}} className="rule"
                    ...
                    <Text
                        hierarchy="tertiary">{Array.isArray((val as any)) ? "is a list of type" : "is a nested object"}</Text>
                </Flex>
            ) : (
                <Flex align="center" style={{gap: ".35rem", textWrap: "nowrap"}} className="rule"
                      aria-selected={isActive || undefined}>
                    {Number.isNaN(Number(key)) ? (
                        <>
                            ...
                            <Text hierarchy="tertiary">is a field of type</Text>
                        </>
                    ) : null}
                    <Badge border
                           color={hashToColor(typeof val === "string" ? "Text" : typeof val === "boolean" ? "Boolean" : "Number")}
                           style={{verticalAlign: "middle"}}>
                        <Text size="xs"
                              style={{color: "inherit"}}>{typeof val === "string" ? "Text" : typeof val === "boolean" ? "Boolean" : "Number"}</Text>
                    </Badge>
                </Flex>
            )
